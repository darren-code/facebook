<template>
    <div class="bg-white rounded shadow w-2/3 p-4">
        <div class="flex justify-between items-center">
            <div>
                <div class="w-8">
                    <img :src="authUser.data.attributes.profile_image.data.attributes.path" alt="Nothing" class="w-8 h-8 object-cover rounded-full">
                </div>
            </div>
            <div class="flex-1 flex mx-4 items-center">
                <input v-model="postMessage" type="text" name="body" class="rounded-full w-full pl-4 h-8 bg-gray-200 focus:outline-none focus:shadow-outline text-sm" placeholder="Add a post" id="body">
                <transition name="fade">
                    <button v-if="postMessage" @click="postHandler" class="bg-gray-200 ml-2 px-3 py-2 rounded-full">Post</button>
                </transition>
            </div>
            <div>
                <button ref="postImage" class="dz-clickable flex justify-center items-center rounded-full w-10 h-10 bg-gray-200 focus:outline-none">
